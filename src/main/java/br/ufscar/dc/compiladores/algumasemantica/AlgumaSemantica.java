...
import static br.ufscar.dc.compiladores.algumasemantica.AlgumaSemanticaUtils.adicionarErrorsSemanticos;
import br.ufscar.dc.compiladores.algumasemantica.TabelaDeSimbolos.TipoDeclaracao;

/**
 *
 * @author ana
 */
public class AlgumaSemantica extends AlgumaBaseVisitor {
    ...
    public Object visitCmdAtribuicao(AlgumaParser.CmdAtribuicaoContext ctx) {
        TipoDeclaracao tipoExpressao = AlgumaSemanticaUtils.verificarTipo(escopo, ctx.expressao());
        boolean error = false;
        ...
        if (tipoExpressao != TipoDeclaracao.INVALIDO) {
            for (TabelaDeSimbolos escopoAtual : escopo.percorrerEscoposAninhados()) {
                ...
                if (escopoAtual.existe(nome)) {
                    TipoDeclaracao tipoVariavel = AlgumaSemanticaUtils.verificarTipo(escopo, nome);
        ...
        if (error)
            AlgumaSemanticaUtils.adicionarErrorsSemanticos(ctx.identificador().start, "atribuicao nao compativel para " + nome);
